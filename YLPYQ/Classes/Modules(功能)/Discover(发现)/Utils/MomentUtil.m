...
+ (NSArray *)getMomentList:(int)momentId pageNum:(int)pageNum
{
    ...
    NSArray * tempList = [YLMoment findByCriteria:sql];
    NSInteger count = [tempList count];
    for (NSInteger i = 0; i < count; i ++)
    {
        YLMoment * moment = [tempList objectAtIndex:i];
        // 处理评论 ↓↓
        ...
        for (NSInteger i = 0; i < count; i ++)
        {
            ...
            YLComment * comment = [YLComment findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",(long)pk]];
            YLUser * user = nil;
            if (comment.fromId != 0) {
                user = [YLUser findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",(long)comment.fromId]];
            } else {
                user = nil;
            }
            comment.fromUser = user;
            if (comment.toId != 0) {
                user = [YLUser findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",(long)comment.toId]];
            } else {
                user = nil;
            }
            comment.toUser = user;
        ...
        {
            NSInteger pk = [[idList objectAtIndex:i] integerValue];
            YLUser * user = [YLUser findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",(long)pk]];
            [list addObject:user];
        }
        ...
        {
            NSInteger pk = [[idList objectAtIndex:i] integerValue];
            YLPicture * pic = [YLPicture findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",(long)pk]];
            [list addObject:pic];
        }
        moment.pictureList = list;
        // 发布者
        YLUser * user = [YLUser findFirstByCriteria:[NSString stringWithFormat:@"WHERE PK = %ld",moment.userID]];
        moment.user = user;
        // 位置
        YLLocation * location = [YLLocation findByPK:1];
...
+ (NSString *)getLikeString:(YLMoment *)moment
{
    ...
    for (NSInteger i = 0; i < count; i ++)
    {
        YLUser * user = [moment.likeList objectAtIndex:i];
        if (i == 0) {
...
+ (void)createData
{
    NSInteger count = [[YLUser findAll] count];
    if (count > 0) {
        return;
    }
    // 名字
    ...
    YLComment * formerComment = nil; // 前一个
    for (NSInteger i = 0; i < max; i ++)
    {
        ...
        [user save];
        // 消息
        YLMessage * message = [[YLMessage alloc] init];
        ...
        [message save];
        // 评论
        YLComment * comment = [[YLComment alloc] init];
        ...
        // 图片
        YLPicture * picture = [[YLPicture alloc] init];
    ...
    // 位置
    YLLocation * location = [[YLLocation alloc] init];
    ...
    {
        // 动态
        YLMoment * moment = [[YLMoment alloc] init];
